...
void game(Chips *, Chips *, int, int, int, int, int []);        //the actual blackjack game
...
int main()
{
    ...
    do{
        shuffle(deck, SIZE);
        game(&player, &bet, Ace, J, Q, K , deck);
        empty = check(&player);
        if(empty)
        {
...
void game(Chips *player, Chips *bet, int Ace, int J, int Q, int K, int deck[])
{
    int hand = 0;
    int dealer = 0;
    int counter = 4;
