 ...
 * also implementing doubling-down
...
//version 5 will try to implement the betting system
//more fleshed out to have a bank class that is aggregated with the player class's constructor
//hopefully i can then implement a vector in the STL library to store each chip
int main()
{
    int cards = 52;
    int temp;
    ...
    string name;
    cout << "Welcome to the Lehr Casino!" << endl;
    cout << "We only have one game and it is blackjack." << endl;
    cout << "Would you like to play?" << endl;
    cin >> yesno;
    if(yesno == 'y' || yesno == 'Y')
    {
        cout << "Great! Now right this way to the bank." << endl;
        cout << "Could you provide your name please?" << endl;
        cin.ignore();
        getline(cin,name);
    }
    else
    {
        cout << "We cannot have you walking around our establishment unless we know your name!" << endl;
        cout << "We must ask you to leave." << endl;
        exit(EXIT_FAILURE);
    }
    
    cout << "How much would you like to buy into the game? (minimum buy-in is $100)" << endl;
    cin >> money;
    ...
    Bank user(money);
    //sets chips depending on how much money user enters
    user.chips();
    //welcome function
    ...
    do{
        try
        {
            if(bet > user.getMoney())
                throw "You cannot bet that much.";
            else
            {
